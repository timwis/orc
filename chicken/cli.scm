...
(receive (options args) (args:parse (command-line-arguments) opts)
  (with-backing-store (get-backing-store (backing-store)) (lambda ()
    (match args
      ...
      (("ensure-key" register-name region-name key-name item-blobs ...)
        (and-let* ((register (open-register register-name))
                   (region (string->symbol region-name))
                   (key (make-key key-name))
                   (needs-entry? (not (register-record-ref register region key)))
                   (items (map make-item item-blobs))
                   (entry (apply make-entry (append (list region key (time->date (current-time))) items)))
                   (register (register-append-entry register entry)))
          ((entry-formatter (current-format)) entry)))

      (("ensure-items" register-name region-name key-name item-blobs ...)
        (and-let* ((register (open-register register-name))
                   (region (string->symbol region-name))
                   (key (make-key key-name)))
          (let* ((current-entry (register-record-ref register region key))
                 (current-items (if current-entry (entry-items current-entry) '()))
                 (new-items (map make-item item-blobs))
                 (item-in-entry? (apply disjoin (map (lambda (item) (cut item-eqv? item <>)) current-items)))
                 (missing-items (remove item-in-entry? new-items)))
            (when (not (null? missing-items))
              (let* ((items (append current-items missing-items))
                     (entry (apply make-entry (append (list region key (time->date (current-time))) items)))
                     (register (register-append-entry register entry)))
                ((entry-formatter (current-format)) entry))))))
